...
namespace Game.Skill
{
    public class RoitDeployer : SkillDeployer
    {
        ...
        private void Update()
        {
            if(flag)
            {
                gameObject.transform.position = new Vector3(gameObject.transform.position.x, SkillData.owner.transform.position.y + (3 + 20 * timer), gameObject.transform.position.z);
            }
            timer += 0.01f;
            Debug.Log(timer);
        }
